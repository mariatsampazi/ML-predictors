...
pred_time_Table = readtable('next_pred_time.csv');

% extracting/saving the predicted throughput 
pred_Table = readtable('next_pred.csv' );
...
% return
dim1=size(throughput_pred,2);
dim2=size(throughput_real,2);
dim3=size(time_pred,2);
dim4=size(time_real,2);
% all are the same! for simplicity reasons we det
dim=dim4;
...
real=movmean(throughput_real2,8500); % real data
pred=movmean(throughput_pred2,9550); % predicted data

figure(1);
plot(time_real2,throughput_real2,'b'); 
hold on; 
title('Moving Averages Plots');
grid on; 
plot(time_pred2,throughput_pred2,'r'); 
legend('Actual','Predicted');


%% -------------do a histogram for the actual throughput data--------------
hist_real=throughput_real;
hist_pred=throughput_pred;
%--------------------------------------------------------------------------
figure(2); hold on; grid on; title('Histogram for the actual downlink throughput');histogram(hist_real);
figure(3); hold on; grid on; title('Histogram for the actual downlink throughput');histogram(hist_pred);

figure(4); plot(time_pred2, abs(throughput_real2-throughput_pred2)); hold on; 
